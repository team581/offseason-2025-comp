...
public class RobotManager extends StateMachine<RobotState> {
 ...
 protected RobotState getNextState(RobotState currentState) {
     return switch (currentState) {
      ...
      case ALGAE_INTAKE_L2_APPROACH, ALGAE_INTAKE_L3_APPROACH -> closeEnoughToReefSide ? currentState.getNextAlgaeIntakeState() : currentState;

      case ALGAE_INTAKE_L2, ALGAE_INTAKE_L3 -> currentState;

      case ALGAE_INTAKE_L2_HOLDING, ALGAE_INTAKE_L3_HOLDING -> cameraOnlineAndFarEnoughFromReef() || drivingFast ? currentState.getNextAlgaeIntakeState() : currentState;
 ...
 protected void collectInputs() {
  ...
  drivingFast = Math.sqrt(Math.pow(robotSpeeds.vxMetersPerSecond,2) + Math.pow(robotSpeeds.vyMetersPerSecond,2)) > 5.0;
 }
