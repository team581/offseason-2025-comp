...
public class GamePieceDetection {
  private static final Pose3d LIMELIGHT_POSE_TO_ROBOT = new Pose3d(0.5,0.0,0.5, new Rotation3d(0,Units.degreesToRadians(20),0));
...
DogLog.log("GamePieceDetection/limelightPose3d", LIMELIGHT_POSE_TO_ROBOT);
  ...
  public static double calculateXAngleOffsetToGamePiece(Pose2d robotPoseAtCapture, GamePieceResult visionResult){
    ...
    return -1* (LocalizationSubsystem.distanceAngleToTarget(new Pose2d(gamePiecePose, new Rotation2d()), robotPoseAtCapture).targetAngle()- robotPoseAtCapture.getRotation().getDegrees());
  }
}
