  ...
  public boolean atGoal() {
    return switch (getState()) {
      case ALGAE_BACKWARD_NET ->
          MathUtil.isNear(WristState.ALGAE_BACKWARD_NET.getAngle(), motorAngle, 1);
      case ALGAE_FORWARD_NET ->
          MathUtil.isNear(WristState.ALGAE_FORWARD_NET.getAngle(), motorAngle, 1);
      ...
      case GROUND_ALGAE_INTAKE ->
          MathUtil.isNear(WristState.GROUND_ALGAE_INTAKE.getAngle(), motorAngle, 1);
      case GROUND_CORAL_INTAKE ->
          MathUtil.isNear(WristState.GROUND_CORAL_INTAKE.getAngle(), motorAngle, 1);
  ...
  protected void afterTransition(WristState newState) {
    switch (newState) {
      ...
      case IDLE -> {
        motor.setControl(
            motionMagicRequest.withPosition(
                Units.degreesToRotations(clamp(WristState.IDLE.getAngle()))));
      }
      case SOURCE_INTAKE -> {
        motor.setControl(
            motionMagicRequest.withPosition(
      ...
      case UNJAM -> {
        motor.setControl(
            motionMagicRequest.withPosition(
                Units.degreesToRotations(clamp(WristState.UNJAM.getAngle()))));
      }
      default -> {}
    }
  }
